  ...
  const handleWalletOrder = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Get transaction/order ID for payment status page
        const transactionId = res.data?.orderId || res.data?.transactionId || res.transaction?.orderId || res.transaction?.transactionId || orderId;
        const clientTxnId = res.data?.clientTxnId || res.transaction?.clientTxnId || transactionId;
        
        // Small delay to ensure balance is updated before navigation
        setTimeout(() => {
          // Navigate to payment status page
          navigate(`/payment-status?client_txn_id=${clientTxnId}${transactionId && transactionId !== clientTxnId ? `&txn_id=${transactionId}` : ''}`);
        }, 500);
      } else {
